...
var manager = null;
$(function (){
	var channelId=$("#id", window.parent.document).val();
    $("#tree1").ligerTree({url:"${ctx}/channel/channelEditShu.do?channelId="+channelId,
    	                   idFieldName: "id", 
...
function selectDemp(){
    ...
    if(notes.length<1){
    	$.ligerDialog.warn("请选择渠道");
    }
    else if($("#checkSelectParentId", window.parent.document).val()==notes[0].data.id){
    	$.ligerDialog.warn("上级渠道不能选择自己");
    }
    else{
    	var id=notes[0].data.id;
    	var name=notes[0].data.channelName;
        $("#parentId", window.parent.document).val(id);
        $("#parentName", window.parent.document).val(name);
